    ...
    <BaseCard className="space-y-5 overflow-hidden bg-gradient-to-br from-ink via-[#18375d] to-[#214870] text-white">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2">
          <Chip className="border-white/10 bg-white/10 text-white">Current streak</Chip>
          <div>
            <h2 className="display-font text-5xl font-semibold leading-none sm:text-6xl">
              {currentStreak} days
            </h2>
            <p className="mt-2 max-w-xs text-sm leading-6 text-white/70 sm:text-base">
              Small daily actions are adding up. One quick check-in keeps the momentum going.
            </p>
          </div>
        </div>
        <div className="rounded-[1.6rem] bg-white/10 px-4 py-4 text-right">
          <p className="text-xs uppercase tracking-[0.22em] text-white/55">Best streak</p>
          <p className="mt-2 text-xl font-semibold">{bestStreak}</p>
        </div>
      </div>

      <div className="rounded-[1.8rem] bg-white/10 p-4 backdrop-blur">

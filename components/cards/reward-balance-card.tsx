  ...
  return (
    <BaseCard className="space-y-4 bg-gradient-to-br from-white/88 via-sky/72 to-mint/78">
      <div className="space-y-2">
        <Chip className="bg-white/82 text-ink/80">Rewards</Chip>
        <div className="flex items-end justify-between gap-3">
          ...
          <div className="text-right">
            ...
            <p className="mt-1 text-xs uppercase tracking-[0.24em] text-ink/60">points</p>
          </div>
        </div>
      </div>

      <div className="rounded-3xl bg-white/82 px-4 py-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm text-ink/72">Today&apos;s reward</p>
            <p className="mt-1 text-sm text-ink/68">Every check-in adds to your running balance.</p>
          </div>
        ...
        {data.history.length > 0 ? (
          <div className="space-y-2">
            {data.history.map((item) => (
              <div
                className="rounded-3xl bg-white/82 px-4 py-3"
                key={item.id}
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="font-medium text-ink">{item.label}</p>
                    <p className="mt-1 text-sm text-ink/72">{item.note}</p>
                  </div>
          ...
          <div className="rounded-3xl bg-white/82 px-4 py-4 text-sm text-ink/72">
